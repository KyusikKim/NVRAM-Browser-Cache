...
/*[sslab cache]*/
QAbstractNetworkCache *QNetworkReplyImplPrivate::networkTopCache() const
{
#ifdef SSLAB__DEBUG
	qWarning("[sslab][%s][tid %lu] %s:%d:%s",QDateTime::currentDateTime().toString("dd.MM.yyy hh:mm:ss.zzz").toAscii().data(), QThread::currentThreadId(), __FILE__,__LINE__,__func__);
#endif
    ...
    if (!backend)
        return 0;
    return backend->networkTopCache();
}
...
void QNetworkReplyImplPrivate::createCache()
{
    ...
    // check if we can save and if we're allowed to
	 if (!networkCache()
//	 if (!networkCache() || !networkTopCache() /*[sslab cache]*/
...
//	return (cacheEnabled && networkCache() && networkTopCache() != 0); /*[sslab cache]*/
}
...
void QNetworkReplyImplPrivate::setCachingEnabled(bool enable)
{
    ...
    if (enable) {
        ...
        networkCache()->remove(url);
		  if (networkTopCache()) networkTopCache()->remove(url); /*[sslab cache]*/
        cacheSaveDevice = 0;
        cacheEnabled = false;
    }
}
        ...
        networkCache()->remove(url);
		 ...
		 if (!networkTopCache())
	        networkCache()->insert(cacheSaveDevice);
			  ...
			  networkTopCache()->insert(cacheSaveDevice); /*[sslab cache]*/
			  networkCache()->expire(); /*[sslab cache]*/
		 }
    }
    cacheSaveDevice = 0;
    cacheEnabled = false;
...
void QNetworkReplyImplPrivate::initCacheSaveDevice()
{
    ...
    if (redirectionTarget.isValid()) {
	 ...
	 cacheSaveDevice = nc->prepare(metaData); /*[sslab cache]*/

    if (!cacheSaveDevice || (cacheSaveDevice && !cacheSaveDevice->isOpen())) {
        if (cacheSaveDevice && !cacheSaveDevice->isOpen())
            qCritical("QNetworkReplyImpl: network cache returned a device that is not open -- "
                  "class %s probably needs to be fixed",
						nc->metaObject()->className()); /*[sslab cache]*/

		  nc->remove(url); /*[sslab cache]*/
        cacheSaveDevice = 0;
        cacheEnabled = false;
    }
}
...
QNetworkReplyImpl::~QNetworkReplyImpl()
{
    ...
    if (d->isCachingEnabled()) {
        d->networkCache()->remove(url());
		  if (d->networkTopCache()) d->networkTopCache()->remove(url()); /*[sslab cache]*/
	 }
