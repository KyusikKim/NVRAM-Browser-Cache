    ...
    requestFinishedSecureCount(0)
{
    ...
    QString location = QDesktopServices::storageLocation(QDesktopServices::CacheLocation);
	 QString tlocation = QDesktopServices::storageLocation(QDesktopServices::TopCacheLocation);
    diskCache->setCacheDirectory(location);
	 diskCache->setMigrateDirectory(tlocation);
	 diskCache->setLastCache(true);	 
	 diskCache->setMaximumCacheSize(1024 * 1024 * 512);
    setCache(diskCache);

	 QNetworkDiskCache *topCache = new QNetworkDiskCache(this);
	 topCache->setCacheDirectory(tlocation);
	 topCache->setMigrateDirectory(location);
	 topCache->setLastCache(false);
	 setTopCache(topCache);
}
