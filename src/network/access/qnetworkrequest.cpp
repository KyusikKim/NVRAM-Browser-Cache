    ...
    : d(new QNetworkRequestPrivate)
{
    ...
    d->url = url;
	strcpy(d->originalUrl, url.toString().toLatin1().data());	//sslab.sr
}
    ...
    : d(other.d)
{
	...
	qWarning("[sslab][%s][tid %lu] %s:%d:%s",QDateTime::currentDateTime().toString("dd.MM.yyy hh:mm:ss.zzz").toAscii().data(), QThread::currentThreadId(), __FILE__,__LINE__,__func__);
#endif
	strcpy(d->originalUrl, other.d->originalUrl);	//sslab.sr
}
...
char* QNetworkRequest::originalUrl()	//sslab.sr
{
	return d->originalUrl;
}

void QNetworkRequest::setOriginalUrl(const char* url)	//sslab.sr
{
	strcpy(d->originalUrl, url);
}
