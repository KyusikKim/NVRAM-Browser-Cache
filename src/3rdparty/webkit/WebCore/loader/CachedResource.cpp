...
//		qWarning(">> m_response is null(return false) -isExpired()");	//ssalb.header
        return false;
	}
	
//	qWarning(">> currentAge() > freshnessLifetime()");	//sslab.header
...
bool CachedResource::mustRevalidate(CachePolicy cachePolicy) const
{
    ...
    if (m_errorOccurred) {
//		qWarning(">> m_errorOccurred(return true) -mustRevalidate()");	//sslab.header
        return true;
	}

    if (m_loading) {
//		qWarning(">> m_loading(return false) -mustRevalidate()");	//sslab.header
        return false;
	}
    
    if (m_response.cacheControlContainsNoCache() || m_response.cacheControlContainsNoStore()) {
//		qWarning(">> m_response(return true) -mustRevalidate()");	//sslab.header
        return true;
	}

    if (cachePolicy == CachePolicyCache) {
//		qWarning(">> CachePolicyCache -mustRevalidate()");	//sslab.header
        return m_response.cacheControlContainsMustRevalidate() && isExpired();
	}
	
//	qWarning(">> isExpired()");	//sslab.header
    return isExpired();
}
