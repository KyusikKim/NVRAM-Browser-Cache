...
/*#include "CString.h"	//sslab.header
#ifndef SSLAB_SR
#define SSLAB_SR
#endif
#include <stdio.h>
...
//static const int cDefaultCacheCapacity = 128;	//sslab.header
...
/*#ifdef SSLAB_SR
		...
		char buff[4096] = {0,};
		sprintf(buff, ">Resource URL: %s< :: Load Start Time: %lld ms \n",url.string().utf8().data(), startTime);
		...
		qWarning("[SR-URLs(Cache)] url:%s, readyUrl:%s",url.string().utf8().data(), readyUrl);
		if (sr_flag == 2 && !strcmp(url.string().utf8().data(), readyUrl)) {
			...
			*shdcacheTime = endTime - m_startTime;
			...
			if (!resource) {
				...
				qWarning("[SR-reload] going to re-load");
				resource->load(docLoader);

				if (sr_flag == 3) {
					endTime = QDateTime::currentDateTime().currentMSecsSinceEpoch();
...
void Cache::evict(CachedResource* resource)
{
	...
	/*sslab.header*/
/*	qWarning("[Header: age] %s",(resource->response().httpHeaderField("age")).utf8().data());
	qWarning("[Header: cache-control] %s",(resource->response().httpHeaderField("cache-control")).utf8().data());
	qWarning("[Header: date] %s",(resource->response().httpHeaderField("date")).utf8().data());
	qWarning("[Header: expires] %s",(resource->response().httpHeaderField("expires")).utf8().data());
	qWarning("[Header: last-modified] %s",(resource->response().httpHeaderField("last-modified")).utf8().data());
	qWarning("[Header: pragma] %s",(resource->response().httpHeaderField("pragma")).utf8().data());*/
