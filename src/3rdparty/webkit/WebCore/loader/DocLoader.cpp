...
//	qWarning(">> [policy_check]URL: %s\n", fullURL.string().utf8().data());	//sslab.header
    switch (cachePolicy()) {
    case CachePolicyVerify:
//		qWarning(">> CachePolicyVerify");	//sslab.header
        if (!existing->mustRevalidate(CachePolicyVerify))
            return;
//		qWarning(">> call revalidateResource()");	//sslab.header
        cache()->revalidateResource(existing, this);
        break;
    case CachePolicyCache:
//        qWarning(">> CachePolicyCache");		//sslab.header
...
//        qWarning(">> CachePolicyReload");	//sslab.header
		cache()->remove(existing);        
        break;
    case CachePolicyRevalidate:
//        qWarning(">> CachePolicyRevalidate");	//sslab.header
		cache()->revalidateResource(existing, this);
        break;
    case CachePolicyAllowStale:
//        qWarning(">> CachePolicyAllowStale");	//sslab.header
		return;
    }
