...
/*
	sslab cache
 */
void QNetworkAccessManager::setTopCache(QAbstractNetworkCache *cache)
{
#ifdef SSLAB__DEBUG
	qWarning("[sslab][%s][tid %lu] %s:%d:%s",QDateTime::currentDateTime().toString("dd.MM.yyy hh:mm:ss.zzz").toAscii().data(), QThread::currentThreadId(), __FILE__,__LINE__,__func__);
#endif
    ...
    Q_D(QNetworkAccessManager);
    if (d->networkTopCache != cache) {
        delete d->networkTopCache;
        d->networkTopCache = cache;
        if (d->networkTopCache)
            d->networkTopCache->setParent(this);
    }
}
